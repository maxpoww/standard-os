...
| **mpris-publisher** | `mpris-publisher.service` (via `home/modules/mpris-publisher.nix`) | `/home/max/mpris-waybar/scripts/mpris-publisher` (out-of-Nix-store, live-edit pattern; repo not git-tracked) | `/tmp/waybar-cache/{mpris-info, mpris-playpause, mpris-volume, mpris-selector, mpris-prev, mpris-next, mpris-output}` (7 pill caches for the future bar pill) + `/tmp/waybar-cache/mpris-snapshot.json` (composite for canvas dashboard, shape: `{active, player, source_label, title, artist, album, status, status_glyph, pos_sec, len_sec, pct, pos_text, len_text, art_path, updated}`) + `/tmp/mpris/current` (atomic art-symlink) | RTMIN+12 (shared with notif-daemon — pill caches only; composite is canvas-only, consumed via inotify by `widgets/scripts/canvas-mpris-listen`) |
...
- ✓ Hypr-bg-daemon (single-rule painter) owns `/tmp/glass-mode` — wallpaper switches to the bg-window's top-edge color when the snapshot has a non-null `bg_window` (the workspace's fullscreen window if any, else the lone non-floating non-pseudo tile); otherwise restores the waypaper image. The `bg_window` is selected by `hypr-context-daemon` so a focused float on top of a tile doesn't break the sample. The trigger is purely topological — `gaps_out` is NOT checked because `hyprctl getoption` returns the global config, not the per-workspace effective value Hyprland applies via `w[tv1]`/`f[1]` selectors. Retires `hypr-edge-bg`, `hypr-activities`, `hypr-dive`, and `glass-text-daemon`.
