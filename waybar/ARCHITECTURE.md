...
- ✓ Hypr-bg-daemon (single-rule painter) owns `/tmp/glass-mode` — wallpaper switches to the bg-window's top-edge color when the workspace has zero gaps and a `bg_window` exists (the workspace's fullscreen window if any, else the lone non-floating non-pseudo tile); otherwise restores the waypaper image. The `bg_window` is selected by `hypr-context-daemon` in the snapshot so a focused float on top of a tile doesn't break the sample. Retires `hypr-edge-bg`, `hypr-activities`, `hypr-dive`, and `glass-text-daemon`.
