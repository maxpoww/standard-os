...
/etc/nixos/home/waybar/scripts/        ← daemon + per-module shell scripts. Bundled into the `waybar-scripts` derivation in modules/waybar.nix (writeShellScriptBin + shellcheck gate). Source-of-truth in git; runtime binaries live in /nix/store/.
/tmp/waybar-cache/                     ← per-module JSON files written by daemons, read by waybar. Also: `hypr-context.json` snapshot for inotify consumers.
/tmp/glass-mode                        ← single-line file: "light" or "dark" (written by hypr-bg-daemon)
/tmp/hypr-edge-bg/                     ← hypr-bg-daemon's solid-color PNG cache (bg_<hex>.png) + waypaper-luminance.json
```
...
Every dynamic exec MUST read `/tmp/glass-mode` and emit `"class": "light"` or `"class": "dark"`. The matching CSS rules in `style.css` flip text color (white ↔ near-black with text-shadow) so the pill stays readable under any wallpaper. This is the **glass-mode contract** (writer: `hypr-bg-daemon`; see next section). Modules that ignore it become invisible over light backgrounds.
...
## The glass-mode writer (system-wide UX feature)

`waybar-hypr-bg-daemon.service` (the single-rule background painter) owns `/tmp/glass-mode`. It knows the current backdrop color first-hand: when the bg trigger rule fires it sampled the focused window's top-edge color; when the rule does not fire it has cached the waypaper image's luminance (`/tmp/hypr-edge-bg/waypaper-luminance.json`). After each apply it computes ITU-R BT.601 luminance and writes `light` or `dark` to `/tmp/glass-mode` atomically.

The retired `glass-text-daemon` polled `/tmp/hypr-edge-bg/bg_<hex>.png` filenames at 2 Hz to infer the same value. The new design folds writer and source into one process, which removes the cross-project coupling (unloading the waypaper PNG used to break glass-text's luminance tracking — a hazard now structurally impossible).

Implementation reference: `/etc/nixos/home/waybar/scripts/hypr-bg-daemon.sh` — `set_glass_mode()` + `apply_color()` + `apply_waypaper()`.
...
systemctl --user restart waybar.service waybar-hypr-context-daemon.service waybar-hypr-bg-daemon.service
...
- **~~Don't unload the waypaper image when cycling solid-color backgrounds.~~** Resolved 2026-06-13: the cross-project hazard between `hypr-edge-bg` and `glass-text-daemon` no longer exists. `hypr-bg-daemon` owns both paint AND `/tmp/glass-mode`; it caches the waypaper's luminance in `waypaper-luminance.json` once per config change, so the waypaper image's preload state doesn't affect glass-mode tracking.
...
- **Hardcoded `"dark"` / `"light"` literal in a class-array emit site = pill never adapts** (2026-06-12). Any printf or array literal that hardcodes the theme token — `local -a classes=("opt-pill" "dark")`, `classes='["opt-pill","dark","opt-no"]'`, `printf '...{"class":["opt-pill","dark"...]...'` — means that pill emits the same theme regardless of `/tmp/glass-mode`. It will be invisible on whichever wallpaper luminance contradicts the hardcoded value. The retired `glass-text-daemon`'s central `update_cache_mode` was supposed to fix this retroactively by rewriting all caches on mode flip — but for ~2 weeks (2026-05-28 → 2026-06-12) its sed regex targeted the obsolete string form `"class":"..."` while every cache was array form `"class":[...]`, so it silently no-op'd. (After 2026-06-13's unification this Layer 1 no longer exists — `hypr-bg-daemon` writes glass-mode but does NOT rewrite waybar caches; the discipline below is now the only defence.) The reliable architecture is now three layers (Layer 1 central jq array surgery + Layer 2A pill_emit auto-substitutes any caller-supplied theme token with fresh `pill_theme()` + Layer 2B emit sites read `/tmp/glass-mode` at write time). **Discipline:** before emitting a class array, read `/tmp/glass-mode` (bash: `cat /tmp/glass-mode 2>/dev/null || printf dark`; bash sourcing pill.sh: `$(pill_theme)`). Sanity grep: `grep -rn '"dark"\|"light"' /etc/nixos/home/scripts/ /etc/nixos/home/modules/ /etc/nixos/modules/ | grep -E '(class|opt-pill)'` should return only comment lines.
- **GTK 3 CSS descendant `label` selector doesn't reliably match the GtkLabel inside a waybar custom module** (2026-06-12). The canonical adaptive-text rule was `.opt-pill.light label { color: opt-text-on-light }` — a descendant selector on `label`. In waybar 0.14.0 / GTK 3 this silently no-op'd for every text-bearing pill (workspaces, window title, clock, battery, win-*, notif-*, dictate, …). The only pills that visibly flipped were the opt-plus pair (`+` launcher and `ws-current` swap face) — and only because *their* `.opt-plus.light` / `.opt-plus.opt-swap.light` rules set `color:` directly on the pill, not on a label descendant. The user had reported "only `+` and workspaces option flip" for weeks; commit 36abfe1's "bulletproof" three-layer Layer-1/2A/2B fix landed the *cache content* correctly (`["opt-pill","light",...]`) but the CSS rule that was supposed to consume it never matched. **The fix:** set `color:` directly on `.opt-pill.light` / `.opt-pill-child.light` so GTK CSS inheritance cascades into the label. **The discipline:** when adding an adaptive-text rule, write it on the pill (`.opt-pill.<state>`), not on `label` descendant — the label inherits via CSS color cascade. Reserve label-direct rules for cases where specificity must beat a pill-direct rule (the `.opt-plus.opt-swap.light:hover label { color: transparent }` rule still works exactly this way — 4 compound classes + 1 element wins over the new pill rule's 2 compound classes on hover). Sanity check: `grep -n '\.light label' style.css` — every match should be a label-direct OVERRIDE of an existing pill-direct rule (today: only the opt-plus-swap hover rule).
- **`hyprctl --batch` does NOT support hyprpaper commands** (2026-06-14). Hyprpaper has its OWN IPC socket (`$XDG_RUNTIME_DIR/hypr/$SIG/.hyprpaper.sock`) separate from Hyprland's main socket (`.socket.sock`). `hyprctl hyprpaper preload X` works because hyprctl knows to route `hyprpaper *` to the hyprpaper socket; in `--batch` mode every command goes to the main socket and `hyprpaper preload X` comes back as `"unknown request"`. hyprctl PRINTS the error to stderr but exits 0, so any redirected-to-`/dev/null` benchmark sees a fast no-op and looks like success. Bit `hypr-bg-daemon`: the 2026-06-14 "batch preload + wallpaper" microopt silently stopped painting (waypaper stuck regardless of bg_window state). Discipline: any hyprpaper command stays sequential (`hyprctl hyprpaper …`); never include them in a `hyprctl --batch` string. When measuring a new hyprctl pattern, redirect stderr to a real file (not `/dev/null`) the first time you run it, and grep for `unknown request`.
- **jq precedence: `,` is LOWER than `|`** (2026-06-14). The filter `[ … ] | @tsv, (.monitors | …)` parses as `[ … ] | (@tsv, (.monitors | …))` — both branches get the array as input, the second one then tries to index a string with key `"monitors"` and exits 5. Always parenthesise multi-output pipelines explicitly: `([ … ] | @tsv), (.monitors | …)`. Bit hypr-bg-daemon: this exited 5 inside `evaluate_and_apply`, and the daemon's `set -euo pipefail` shell took down the whole process at startup (systemd marked it failed within 223 ms with no journal output because the failure was inside a process substitution).
- **Hyprland's `pseudo` dispatcher emits NO window events** (2026-06-14). `hyprctl dispatch pseudo` toggles the property and resizes the window but emits only `openlayer>>hyprpaper` + `closelayer>>hyprpaper` (hyprpaper re-render side effects) — no `movewindow`, no `activewindow`, no `changefloatingmode`. A daemon subscribed only to window-level socket2 events will not wake up for pseudo toggles on quiet windows and its snapshot will go stale (the `pseudo`/`w`/`h` fields lag the real state until something else fires). Bit hypr-context-daemon: the topology check excluded pseudo from `bg_window` correctly, but quiet-window pseudo toggles never refreshed the snapshot so the painter kept painting with the old non-pseudo geometry. Fix: subscribe to `openlayer|closelayer` in addition to the window-event set. Cost is low because `emit_snapshot` dedupes unchanged content. Discipline: any new state we want to track from `hyprctl clients` that isn't carried by a `move|active|float|fullscreen|workspace|title` event needs to be considered against the layer-event fallback or polling.
- **`hyprctl getoption general:gaps_out` returns the GLOBAL config, NOT the per-workspace effective value** (2026-06-14). The `w[tv1]` / `f[1]` workspace selectors in `hypr/modules/Workspace_Rules.conf` set `gapsOut: 0` at render-time for matching workspaces, but `hyprctl getoption` still reports `"2 6 6 6"` (the user's global default). Bit `hypr-bg-daemon`: the 2026-06-13 ship checked `workspace.gaps_out == 0` as a "guard" on top of the topology check, which meant the painter never fired in normal use (gaps_out was always 6). The discipline: when you need to know "is this workspace in a gap-collapsed rendering state?", INFER from window topology (single non-floating non-pseudo tile, OR fullscreen window) rather than asking `getoption`. The `bg_window` field on the context-daemon snapshot already encodes this inference. Legacy `hypr-edge-bg` (deleted) had this comment in its decide(): *"a single tiled (non-floating, non-pseudo) window owns the screen → no visible gaps regardless of the Hyprland gaps_in/gaps_out setting."* Do not re-add a `gaps_out` check thinking it's free.
- **`IFS=$'\t' read` collapses leading tab runs even with IFS set explicitly** (2026-06-14). Bash treats space/tab/newline as IFS-whitespace regardless of whether IFS was set to just one of them, so `IFS=$'\t' read -r a b c d e <<<$'\t\t\t9\t3,9'` parses as `a=9, b=3,9, c=, d=, e=` — leading empty fields collapse, every subsequent field shifts left by the run length. Bit `hypr-context-daemon` on empty workspaces: `j/activewindow` returns `{}`, so the daemon's packed `(class, title, addr, ws_current, ws_list_csv) | @tsv` produced `\t\t\t9\t3,9`, which bash parsed as `class=9, title=3,9, …` — the TASK center pill showed the ws-list-in-use as its title, and ws-current fell to `"0"` via `${ws_current:-0}`. On populated workspaces all five fields were non-empty so the bug stayed invisible. **Fix:** when ANY packed field can be empty, use a non-whitespace separator — Unit Separator `\x1F` is the standard pick (jq: `[...] | join("")`, bash: `IFS=$'\x1f' read -r …`). `@tsv` is safe ONLY when every field is guaranteed non-empty. Sanity check: when adding a new packed-fields parse, dry-run the all-empty-leading case before shipping.
